...
async function webSearch(
	...
	if (exaKey) {
		...
		if (response.ok) {
			...
			return data.results.map((r: any) => ({ title: r.title, url: r.url, content: r.content?.text || "" }));
		}
	}
	...
	if (braveKey) {
		...
		if (response.ok) {
			const data = await response.json();
			return (data.web?.results || []).map((r: any) => ({ title: r.title, url: r.url, content: r.description || "" }));
		}
	}
