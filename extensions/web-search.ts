 ...
 *
 * A focused web search extension that provides search and content fetching
 * without unused tools. Uses web search APIs to find information and
 * fetch_content to retrieve full page content.
 */
...
export default function webSearchExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"Use fetch_content when you need to extract specific information from a URL",
		],
	...
	pi.registerCommand("websearch", {
		...
		handler: async (args, ctx) => {
			if (!args) {
				ctx.ui.notify("Web search extension active. Tools: web_search, fetch_content", "info");
				return;
			}
